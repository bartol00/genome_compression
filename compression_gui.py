...
def run_compression():    
    if input_file and output_dir:
        sequence_type = option_var.get()
        program_output.set("")
        result = subprocess.run(['./compression.exe', input_file, sequence_type, output_dir], capture_output=True, text=True)
        print(result.stdout)
...
Label(radio_frame, text="Select sequence type:").grid(row=0, column=0, padx=5)
option_var = StringVar(value="dna")
Radiobutton(radio_frame, text="DNA", variable=option_var, value="dna").grid(row=0, column=1, padx=5)
Radiobutton(radio_frame, text="RNA", variable=option_var, value="rna").grid(row=0, column=2, padx=5)


file_frame = Frame(root)
file_frame.pack(pady=10)

Button(file_frame, text="Select input FASTA file", command=define_input_file, width=20).grid(row=0, column=0, padx=5, pady=5, sticky="ew")
...
Button(run_frame, text="Run compression", command=run_compression).grid(row=0, column=0, padx=5, pady=30)
